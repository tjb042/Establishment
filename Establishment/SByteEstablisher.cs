...
namespace Establishment {
    ...
    public class SByteEstablisher : BaseStructEstablisher<sbyte> {
        ...
        public SByteEstablisher IsGreaterThan(sbyte threshold) {
            if (Value <= threshold) {
                HandleFailure(new ArgumentException("sbyte value must be greater than " + threshold.ToString()));
            }
        ...
        public SByteEstablisher IsGreaterThanOrEqualTo(sbyte threshold) {
            if (Value < threshold) {
                HandleFailure(new ArgumentException("sbyte value must be greater than or equal to " + threshold.ToString()));
            }
        ...
        public SByteEstablisher IsLessThan(sbyte threshold) {
            if (Value >= threshold) {
                HandleFailure(new ArgumentException("sbyte value must be less than " + threshold.ToString()));
            }
        ...
        public SByteEstablisher IsLessThanOrEqualTo(sbyte threshold) {
            if (Value > threshold) {
                HandleFailure(new ArgumentException("sbyte value must be less than or equal to " + threshold.ToString()));
            }
